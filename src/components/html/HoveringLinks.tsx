  ...
  return  (
    <Html center position={[-0.5,2.8,0]}>
      <div className="text-right text-5xl text-white w-72 bg-transparent p-8">
        {isOpen && (
          <div onClick={() => setIsOpen(!isOpen)} className="text-xs w-screen text-green-700 bg-transparent p-8">
        ...
        {!isOpen && <button className="text-xs mb-32 " onClick={() => setIsOpen(!isOpen)}>credits</button>}
      </div>
    </Html>
  );
